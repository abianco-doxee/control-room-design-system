...
It is three things at once: a **published component library** (`@abianco-doxee/cr-*`
...
package under the `@abianco-doxee/cr-*` scope; the root `@abianco-doxee/cr-design-system`
is a convenience umbrella that re-exports every subpath, so existing
`@abianco-doxee/cr-design-system/<subpath>` imports keep working unchanged.

```
control-room-design-system/
├── packages/
│   ├── tokens/        # @abianco-doxee/cr-tokens   — tokens.json (source of truth) → CSS bundle,
│   │                  #   per-theme layers, structure layer, Tailwind @theme, DTCG, theme contract;
│   │                  #   author brands without forking via brands/*.json
│   ├── styles/        # @abianco-doxee/cr-styles   — components.css bundle + base + per-component
│   │                  #   parts/*.css (import-on-use) + the Tailwind v4 entry   (depends on tokens)
│   ├── utils/         # @abianco-doxee/cr-utils    — cn · href · duration · position · time-scale ·
│   │                  #   forms (ArkType ⇄ JSON Schema) · theme runtime  (framework-agnostic)
│   ├── icons/         # @abianco-doxee/cr-icons    — Iconify build tooling + path-data packs (./pixel)
│   ├── components/    # @abianco-doxee/cr-components — 80 Mitosis .lite.tsx → React/Vue/Svelte/Angular/
│   │                  #   Solid/Qwik; per-framework exports (./react …)  (depends on icons)
│   ├── mcp/           # @abianco-doxee/cr-mcp      — Model Context Protocol server (npx-runnable)
│   └── docs/          # @abianco-doxee/cr-docs     — Astro + Starlight site, gallery, component browser,
...
import "@abianco-doxee/cr-tokens/css";        // the token layer (all four themes)
import "@abianco-doxee/cr-styles/components"; // the component styles (or import parts/<name>.css on use)
```
...
```html
<link rel="stylesheet" href="@abianco-doxee/cr-design-system/css" />        <!-- tokens (first) -->
<link rel="stylesheet" href="@abianco-doxee/cr-design-system/components" />  <!-- components -->
...
- **`@abianco-doxee/cr-mcp`** — a Model Context Protocol server: `npx @abianco-doxee/cr-mcp`.
...
- **Token-first.** A single token layer (`@abianco-doxee/cr-tokens`) drives four themes
...
npm run build:tokens     # @abianco-doxee/cr-tokens → CSS + tw-theme + flat + DTCG + theme-contract
...
[`@abianco-doxee/cr-skill`](packages/skill).
...
```bash
npx @abianco-doxee/cr-skill            # → ./.claude/skills (this project)
npx @abianco-doxee/cr-skill --global   # → ~/.claude/skills (all projects)
...
(`@abianco-doxee/cr-tokens`'s `design-tokens/control-room.tokens.json`, with the same
...
- **Source of truth:** the reference Markdown in `references/` and the token source in
  `@abianco-doxee/cr-tokens` (`packages/tokens/tokens/tokens.json`).
