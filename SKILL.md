...
| --- | --- |
| `references/getting-started.md` | **Installing and wiring it up** — the `@abianco-doxee/cr-*` packages, GitHub Packages auth, CSS import order, theme selection, and per-framework imports. |
...
| `references/theming.md` | **Theming & branding** — the feature ⇄ appearance split, the theme contract, authoring a brand (`brands/*.json` + `@abianco-doxee/cr-design-system/theme`), contrast, and the appearance-separation guard. |
...
| `packages/tokens/tokens/tokens.json` (`@abianco-doxee/cr-tokens/tokens.json`) | Machine-readable token source (parse this to generate CSS/Tailwind/JSON). |
| `@abianco-doxee/cr-tokens/css` (`@abianco-doxee/cr-design-system/css`) | Ready-to-use CSS custom properties for all four themes. Import first. |
| `@abianco-doxee/cr-styles/components` (`@abianco-doxee/cr-design-system/components`) | The shipped component layer — `cr-`prefixed classes (`.cr-panel`, `.cr-btn`, …) built on the tokens. Import after the tokens. |
...
- **MUST** build on the token layer (`@abianco-doxee/cr-tokens/css`). Never hardcode a
...
<link rel="stylesheet" href="@abianco-doxee/cr-design-system/css" />
<link rel="stylesheet" href="@abianco-doxee/cr-design-system/components" />
...
`references/components.md`; the class list lives in `@abianco-doxee/cr-styles`
(`components.css`).
