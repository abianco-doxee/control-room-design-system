...
export function isJsonSchema(schema) {
  return (
    ...
    (schema.type === "object" || schema.properties || schema.$schema ||
      schema.allOf || schema.oneOf || schema.anyOf || schema.$ref || schema.$defs || schema.definitions)
  );
}
...
function buildFields(schemaObj, overrides, prefix, root) {
  ...
  const props = resolved.properties || {};
  const required = new Set(resolved.required || []);
  return Object.keys(props).map((n) => buildField(n, props[n], required.has(n), overrides, prefix, root));
}
