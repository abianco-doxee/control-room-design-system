...
The packages are published **privately** under the `@abianco-doxee` scope, so npm
needs a token with `read:packages` before it can resolve them. Add this to your
project's `.npmrc`:

```ini
@abianco-doxee:registry=https://npm.pkg.github.com
...
pnpm add @abianco-doxee/cr-tokens @abianco-doxee/cr-styles

# optional: compiled components for your framework
pnpm add @abianco-doxee/cr-components
```
...
| --- | --- |
| `@abianco-doxee/cr-tokens` | The token layer — theme CSS, a Tailwind `@theme` layer, DTCG export, and the machine-readable theme contract. The source of truth for every value. |
| `@abianco-doxee/cr-styles` | The `cr-` component CSS: 70+ parts, framework-agnostic. Works with plain HTML. |
| `@abianco-doxee/cr-components` | Interactive components compiled from one Mitosis source to React, Vue, Svelte, Angular, Solid, and Qwik. |
| `@abianco-doxee/cr-icons` | The pixel icon set. |
| `@abianco-doxee/cr-utils` | Helpers — `cn`, theme switching, duration/time-scale formatting, form validation. |
| `@abianco-doxee/cr-mcp` | An MCP server exposing the catalog, theme contract, and these docs as tools — so an agent can query the system directly. |
...
```js
import "@abianco-doxee/cr-tokens/css";          // all four themes + primitives
import "@abianco-doxee/cr-styles/components";   // the cr- component classes
```
...
`@abianco-doxee/cr-tokens/themes/*`.
...
```jsx
// React
import { CrButton, CrCombobox } from "@abianco-doxee/cr-components/react";
```

```vue
<!-- Vue -->
<script setup>
import { CrButton, CrCombobox } from "@abianco-doxee/cr-components/vue";
</script>
```
...
```css
@import "tailwindcss";
@import "@abianco-doxee/cr-tokens/tw-theme.css";
```
...
- **MCP server** — `npx @abianco-doxee/cr-mcp` exposes the component catalog, the
...
- **`catalog.json`** — all 83 components with categories, tokens, variants, and
  keywords, queryable.
- **DTCG tokens** — `@abianco-doxee/cr-tokens/dtcg` for design-tool interop.
