...
Decision (locked): **each package publishes independently** under the `@alebianco/cr-*`
scope, owning its own `dist/`, its own `exports`, deps and version. The root
`@alebianco/cr-design-system` stays as a convenience umbrella that re-points its
subpath exports (`./css`, `./theme`, `./react`, …) into the sub-packages, so existing
`@alebianco/cr-design-system/*` consumers keep working unchanged. Cross-package
references use `@alebianco/cr-<pkg>` specifiers (resolved through the workspace
symlinks), never relative `../` hops across package boundaries.

Progress:
- [x] Stage 1 — scaffold workspaces (`workspaces: ["packages/*"]`, stub manifests).
- [x] Extract `@alebianco/cr-utils` (cn · href · duration · position · time-scale ·
      forms · theme). Pure source move; green.
- [x] Extract `@alebianco/cr-tokens` (tokens.json · brands · build-tokens/theme/palette
...
- [x] Extract `@alebianco/cr-styles` (components.css authored bundle · base.css + parts/*
      partials · tailwind.css entry · build-styles). Depends on `@alebianco/cr-tokens`
      (tailwind.css imports `@alebianco/cr-tokens/tw-theme.css`). Green.
- [x] Extract `@alebianco/cr-icons` (Iconify build tooling + path-data packs; ships the
      pixel pack, exports `./pixel`). Bake-in model: `build-barrels` vendors the pack from
      `@alebianco/cr-icons` into every target so `CrIcon`'s relative import resolves and the
...
- [x] Extract `@alebianco/cr-components` (all 80 `.lite.tsx` + overrides + `lib/pt.ts` +
      ...
      ./qwik ./frameworks/*); depends on @alebianco/cr-icons. compile-mitosis resolves the
...
- [x] Extract `@alebianco/cr-docs` (private, dev-only leaf): astro/starlight site (src,
      ...
      suites (+ visual snapshot baselines). Consumes the sibling packages via `@alebianco/cr-*`
      specifiers (starlight-theme.css imports `@alebianco/cr-tokens/css`) and repo-root reads
...
All six workspaces are extracted and independently publishable under `@alebianco/cr-*`
(`utils`, `tokens`, `styles`, `icons`, `components`) plus the private `docs` site. The root
`@alebianco/cr-design-system` remains a convenience umbrella re-exporting every subpath into
the packages, so existing `@alebianco/cr-design-system/*` consumers are unchanged. Cross-
package references use `@alebianco/cr-<pkg>` specifiers throughout.
...
- **Registry.** `.npmrc` maps `@alebianco:registry=https://npm.pkg.github.com`, and
...
owner is the user `alebianco`. The scope is now `@alebianco` (packages
`@alebianco/cr-*`), which matches the owner, so no org move is required. Packages
...
- **Pre-split changeset queue — resolved.** The ~85 changesets predating the split named
  `@alebianco/cr-design-system` (the private workspace root, which is not a workspace
...
could never publish from a repo owned by the user `alebianco`. Renaming the scope to
`@alebianco` — rather than creating a `control-room` org — resolves this with no infra
...
- **One baked icon family.** `CrIcon`'s portable single-`<path>` model (no `innerHTML`, so
  ...
  as *baked* sets. Multi-family is delivered two ways instead: `@alebianco/cr-icons` exposes
...
Done since the split: `build:tw` now emits `@alebianco/cr-styles/utilities.css` (was an
...
│  ├─ styles/                   # @alebianco/cr-styles  (depends on tokens)
│  │   styles/components.css (authored) · build/build-styles.mjs → base.css + parts/*
│  ├─ utils/                    # @alebianco/cr-utils   (cn · href · duration · position · time-scale · forms · theme)
│  ├─ icons/                    # @alebianco/cr-icons   (icon DATA + Iconify build tooling)
│  │   lib/icons/*.ts (pixel + future families) · build/build-icons.mjs
│  │   → per-pack path-data modules, one per Iconify set, keyed by name
│  ├─ components/               # @alebianco/cr-components (depends on tokens, styles, utils, icons)
...
│  │   → dist/frameworks/* · dist/pkg/*  (CrIcon renders; reads packs from @alebianco/cr-icons)
│  └─ docs/                     # @alebianco/cr-docs (private) — astro/starlight
...
`@alebianco/cr-icons` isolates that growth (and the Iconify devDeps) from the
component runtime, and lets an app consume raw icon path data directly. `CrIcon`
stays in `@alebianco/cr-components` and depends on `@alebianco/cr-icons` for the
...
3. **Extract `tokens`.** Move token sources + `build-tokens`/`build-theme`/palette.
   Re-point `verify:tokens/theme/palette`. Consumers import `@alebianco/cr-tokens/css`.
...
6. **Extract `icons`** (leaf data + tooling). Move `lib/icons/` + `build-icons.mjs`;
   ...
   `@alebianco/cr-icons` instead of `../lib/icons/…`; `build-barrels` copies the pack
