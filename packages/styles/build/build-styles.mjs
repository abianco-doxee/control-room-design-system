...
export function nameOf(headerLine) {
...
/**
 * Header name → routing target. THE authoritative mapping: the build routes by
 * it, and `tests/style-parts.test.mjs` imports it to check that no rule is filed
 * under the wrong header. Never re-derive routing from class-name spelling —
 * many families are not spelled like their slug (`.cr-btn` → `button`,
 * `.cr-sev` → `shape`, `.cr-row` → `session-row`).
 */
...
// Only build when run as a script: the routing helpers above are imported as a
// library by tests/style-parts.test.mjs, which must not write any files.
