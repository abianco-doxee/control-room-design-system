...
under `data/`, so `npx @abianco-doxee/cr-mcp` runs with no repo checkout.
...
{
  "mcpServers": {
    "control-room": { "command": "npx", "args": ["-y", "@abianco-doxee/cr-mcp"] }
  }
}
```

Or directly: `npx @abianco-doxee/cr-mcp` (stdio).
...
`npm run build -w @abianco-doxee/cr-mcp` (guarded by `npm run verify -w @abianco-doxee/cr-mcp`).
