...
//            context() → the `export declare …` for the app-level CrContext }
//
// The context is a DIFFERENT SHAPE per target because each framework's own context
// primitive is: React/Solid a context object, Vue/Svelte a `{ cr, key }` pair keyed
// by Symbol, Qwik a ContextId, Angular an injectable class. Typing it per target is
// what lets a consumer provide the global tier without casting.
const TARGETS = {
  vue: {
    ext: "vue",
    value: (n, hasProps) =>
      `export declare const ${n}: import("vue").DefineComponent<${hasProps ? n + "Props" : "Record<string, any>"}>;`,
    context: () => `export declare const CrContext: { cr: CrGlobalConfig; key: symbol };`,
  },
  svelte: {
    ext: "svelte",
    value: (n, hasProps) =>
  ...
  solid: {
    ...
    context: () => `export declare const CrContext: import("solid-js").Context<CrGlobalConfig>;`,
  },
  angular: {
    ext: "js",
    ...
    // An @Injectable({providedIn:"root"}) singleton: consumers mutate its fields
    // (or override the provider) rather than wrapping a subtree.
    context: () =>
      `export declare class CrContext implements CrGlobalConfig {\n  pt: Record<string, CrPassThrough>;\n  locale: string;\n  messages: Record<string, string | ((value: any) => string)>;\n}`,
  },
  qwik: {
    ext: "tsx",
    value: (n, hasProps) =>
    ...
    context: () =>
      `export declare const CrContext: import("@builder.io/qwik").ContextId<CrGlobalConfig>;`,
  },
};
...
const SHARED_TYPES = extractTypeBlocks(readFileSync(join(ROOT, "lib", "pt-types.ts"), "utf8")).map(
  (b) => b.text
);
...
for (const [target, { ext, value, context }] of Object.entries(TARGETS)) {
  ...
  // The context declaration is emitted only when the target actually shipped a
  // context module, so a generator change that drops it shows up as a missing
  // export rather than a declaration pointing at nothing.
  const hasContext = readdirSync(compDir).some(
    (f) => f.includes(".context.") && !f.endsWith(".map")
  );
  const contextDecl = hasContext && context ? context() : "";
