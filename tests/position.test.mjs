...
test("bottom-start: below the anchor, left edges aligned", () => {
  const p = computePosition(anchor(100, 100), floating(), VP, { placement: "bottom-start", offset: 6 });
...
test("flips to top when there's no room below but room above", () => {
  const p = computePosition(anchor(100, 720), floating(200, 120), VP, { placement: "bottom-start" });
...
test("does NOT flip when the preferred side fits", () => {
  const p = computePosition(anchor(100, 100), floating(200, 120), VP, { placement: "bottom-start" });
  assert.equal(p.placement, "bottom-start");
});

test("shift: clamps a right-aligned panel back into the viewport", () => {
  // anchor near the right edge, wide panel would overflow right
  const p = computePosition(anchor(960, 100, 30, 30), floating(300, 100), VP, { placement: "bottom-end", padding: 8 });
...
test("align=center centers the floating element on the anchor", () => {
  const p = computePosition(anchor(400, 100, 100, 30), floating(200, 80), VP, { placement: "bottom-center" });
  assert.equal(p.x, 400 + 50 - 100, "centered: ax + aw/2 - fw/2 = 350");
});

test("horizontal side: right places to the anchor's right", () => {
  const p = computePosition(anchor(100, 100, 80, 30), floating(120, 60), VP, { placement: "right-start" });
  assert.equal(p.x, 100 + 80 + 6, "to the right of the anchor");
  assert.equal(p.y, 100, "top-aligned");
});

test("flip can be disabled", () => {
  const p = computePosition(anchor(100, 720), floating(200, 120), VP, { placement: "bottom-start", flip: false });
