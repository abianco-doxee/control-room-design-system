...
test("components.css carries no brand colour (only physical black/white)", () => {
  ...
  assert.deepEqual(
    fns,
    [],
    `colour-function literals in components.css: ${[...new Set(fns)].join(", ")}`
  );
});
...
test("no component source uses a colour-function literal", () => {
  ...
  for (const f of files) {
    ...
    assert.deepEqual(
      fns,
      [],
      `${f} uses a colour-function literal: ${[...new Set(fns)].join(", ")}`
    );
  }
});
...
test("only the generative-canvas components embed palette — and only as var fallbacks", () => {
  ...
  for (const name of GENERATIVE) {
    ...
    assert.match(
      src,
      /getPropertyValue\(/,
      `${name} must read CSS custom properties (theme-driven), not just hardcode`
    );
  }
});
